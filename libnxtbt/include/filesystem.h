  ...
  class Filesystem{
    public:
      ...
      */
       ...
       * @param reply [true = require reply from NXT; false = no reply from NXT]
      */
      void delete_file(std::string file_name, bool reply=false);
      ...
      */
      void download_file(std::string pc_name, std::string nxt_name);
      ...
      */
      void upload_file(std::string pc_file, std::string nxt_file);
      ...
      */
      unsigned int create_file_list(std::string wild_card);
      ...
      */
       ...
       */
      Nxt_file open_file(std::string file_name, File_mode mode, unsigned int file_size=0);

      /**
       * read a byte std::string from a file
      ...
      */
      ...
      /**
       * write a byte std::string to a file
    ...
    private:
      ...
      int get_first(std::string wild_card, Nxt_file *file);
      unsigned int update_file_list(std::string wild_card);
      ...
      */
      ...
      Nxt_file open_read(std::string file_name);
      Nxt_file open_append(std::string file_name);
      Nxt_file open_write(std::string file_name, unsigned int file_size, char type);
