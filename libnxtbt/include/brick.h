...
class Brick{
  public:
     ...
     * @param message [std::string to be send to the inbox (maximum 57 characters)]
     * @param inbox [specifies the inbox on the NXT(0-9)]
     * @param reply [true = require reply from NXT; false = no reply from NXT]
    */
    void write_msg(std::string message, int inbox, bool reply=false);

    /**
     * Read a message from the NXT's mailbox
     * @return returns a std::string from the inbox specified by inbox (maximum 57 characters)
     * @param inbox [specifies the inbox on the NXT(0-9)]
     * @param remove [true = remove the std::string from the inbox; false = leave the std::string in the inbox]
    */
    std::string read_msg(int inbox, bool remove);
     ...
     * @param reply [true = require reply from NXT; false = no reply from NXT]
    */
    void set_name(std::string name, bool reply=false);
    ...
    */
     ...
     * @param reply [true = require reply from NXT; false = no reply from NXT]
    */
    void play_soundfile(std::string file, bool loop, bool reply=false);
    ...
    */
     ...
     * @param reply [true = require reply from NXT; false = no reply from NXT]
    */
    void start_program(std::string file, bool reply=false);
    ...
    */
    std::string get_current_program();
